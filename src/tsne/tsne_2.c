...
static void compute_gradient(double *P, double *Q, double *Y, double *dY, int n, int dy)
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        for (int j = 0; j < n; j++)
        {
            ...
            dY[i * dy + 0] += y_diff0 * PQ_diff_times_p1_inv;
            dY[i * dy + 1] += y_diff1 * PQ_diff_times_p1_inv;
        }
    }
}
...
uint64_t tsne_2_flops(tsne_data input, tsne_hyperparameters hyperparams) {
    uint64_t n = input.n;
...
void tsne_2(tsne_data input, tsne_hyperparameters hyperparams)
{
